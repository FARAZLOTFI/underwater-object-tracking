...
from src.scuba_tracking.scuba_tracking.models.utils.utils import xyxy2xywh, xywh2xyxy
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        w = img.shape[1]
        h = img.shape[0]
        ##### here we add an additional channel to the input so that we can process it with our MLSD model
        ##img = np.concatenate([img,np.ones([int(h), int(w), 1])], axis=-1)
