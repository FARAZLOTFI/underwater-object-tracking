...
def compute_loss(p, targets, model, nc = 1, hyp = None):  # predictions, targets, model
    ft = torch.cuda.FloatTensor if p[0].is_cuda else torch.Tensor
    lcls, lbox, lobj = ft([0]), ft([0]), ft([0])
    tcls, tbox, indices, anchor_vec = build_targets(model, targets, nc=nc)

    h = hyp#model.hyp  # hyperparameters
    arc = 'default'#model.arc  # # (default, uCE, uBCE) detection architectures
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:  # number of targets
            ...
            # avval tasvir ro peyda kon baad ino bendaz roosh
            # GIoU
            ...
            if 'default' in arc and nc > 1:  # cls loss (only if multiple classes)
        ...
        if 'default' in arc:  # separate obj and cls
            ...
            lcls += CE(pi[..., 4:].view(-1, nc + 1), t.view(-1))
...
def build_targets(model, targets, IOU_THRESH = 0.3, nc=80):
    ...
    for i in model.yolo_layers:
        ...
        if nt:
            ...
            if use_best_anchor:
                ...
                a = torch.arange(na).view((-1, 1)).repeat([1, nt]).view(-1).cuda()
            ...
            reject = False
            if reject:
                j = iou > IOU_THRESH  # iou threshold hyperparameter
        ...
        if c.shape[0]:  # if any targets
            assert c.max() <= nc, 'Target classes exceed model classes'

    return tcls, tbox, indices, av


def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.5, max_num_of_boxes=80):
    """
    ...
    min_wh, max_wh = 0.01, 30000  # (pixels) minimum and maximium box width and height
        ...
        for image_i, pred in enumerate(prediction):
            ...
            for c in pred[:, -1].unique():
                ...
                if n == 1:
                    ...
                    continue
                elif n > max_num_of_boxes:
                    dc = dc[:max_num_of_boxes]  # limit to first 100 boxes: https://github.com/ultralytics/yolov3/issues/117
